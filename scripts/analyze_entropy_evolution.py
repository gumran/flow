...
model_path = "/scratch/inath/checkpoints/tinystories_campbell_masked_flow_5_best_model.pt"
...
dataset_path = "/scratch/inath/datasets/tinystories_5_dataset"
...
timesteps = np.linspace(0, 1, 11)  # [0, 0.1, 0.2, ..., 1.0]
...
with torch.no_grad():
    for t_val in tqdm(timesteps, desc="Processing timesteps"):
        ...
        trained_entropies.append(entropy_trained.cpu())
        ...
        empirical_entropies.append(entropy_empirical.cpu())

# Convert to numpy for plotting
trained_entropies = torch.stack(trained_entropies).numpy()  # (num_timesteps, bs, c)
empirical_entropies = torch.stack(empirical_entropies).numpy()  # (num_timesteps, bs, c)

# %%
# Compute statistics
print("\n=== Summary Statistics ===")

# Average entropy over all positions and examples
trained_mean_entropy = trained_entropies.mean(axis=(1, 2))  # (num_timesteps,)
empirical_mean_entropy = empirical_entropies.mean(axis=(1, 2))  # (num_timesteps,)
...
plt.title('Average Entropy Evolution Over Time', fontsize=14)
...
for idx, pos in enumerate(selected_positions):
    ...
    trained_pos_entropy = trained_entropies[:, :, pos].mean(axis=1)
    empirical_pos_entropy = empirical_entropies[:, :, pos].mean(axis=1)
...
plt.suptitle('Entropy Evolution at Specific Token Positions', fontsize=14)
plt.tight_layout()
...
# Average over examples for heatmap
trained_heatmap = trained_entropies.mean(axis=1).T  # (c, num_timesteps)
empirical_heatmap = empirical_entropies.mean(axis=1).T  # (c, num_timesteps)
...
axes[0].set_title('Trained Model Entropy Heatmap', fontsize=13)
...
axes[1].set_title('Empirical DFM Entropy Heatmap', fontsize=13)
...
plt.title('Difference in Average Entropy Between Models', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
