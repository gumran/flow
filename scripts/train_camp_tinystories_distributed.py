...
if accelerator.is_main_process:
    ...
    wandb.init(
        ...
        name=f"campbell_masked_flow_{subset_size or 'full'}_distributed",
        config={
            "model": "MaskedFMModel",
            "subset_size": subset_size,
...
checkpoint_save_path = "/scratch/inath/checkpoints/"
...
while step < total_steps:
    for batch in dataloader:
        ...
        # Memory tracking only on main process
        if accelerator.is_main_process:
        ...
        if loss.item() < best_loss - min_delta:
            ...
            if accelerator.is_main_process and step - last_save_step >= save_interval:
                ...
                accelerator.save(unwrapped_model.state_dict(), checkpoint_save_path + f"tinystories_campbell_masked_flow_{subset_size or 'full'}_best_model.pt")
                last_save_step = step
        else:
            # steps_no_improve += 1
            pass
        ...
        # tqdm update only on main process
...
if accelerator.is_main_process:
    unwrapped_model = accelerator.unwrap_model(model)
    accelerator.save(unwrapped_model.state_dict(), checkpoint_save_path + f"tinystories_campbell_masked_flow_{subset_size or 'full'}_final_model.pt")
    wandb.finish()
    # %%
